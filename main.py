...
def main() -> None:
    ...
    # برای اطمینان از نمایش صحیح، کلید ذخیره‌شده در پایگاه داده را بررسی می‌کنیم
    # و در صورت نیاز اصلاح می‌کنیم
    for country_code in list(db.ipv4_data.keys()):
        if country_code.lower() == "saudi_arabia":
            country_data = db.ipv4_data[country_code]
            db.ipv4_data["saudi"] = country_data
    ...
    dp.add_handler(CallbackQueryHandler(cb_user_account, pattern='^user_account$')) #Added user account handler
    dp.add_handler(CallbackQueryHandler(cb_subscription_status, pattern='^subscription_status$'))

    # هندلرهای جدید برای مدیریت کاربران
    dp.add_handler(CallbackQueryHandler(cb_admin_manage_users, pattern='^admin_manage_users$'))
    dp.add_handler(CallbackQueryHandler(cb_admin_disable_user, pattern='^admin_disable_user$'))
    dp.add_handler(CallbackQueryHandler(cb_admin_enable_user, pattern='^admin_enable_user$'))

    # هندلرهای جدید برای مدیریت لوکیشن‌ها
    ...
    # هندلرهای جدید برای حذف آدرس IP
    ...
    # Handler registered through ConversationHandler
...
def cb_remove_ip(update: Update, context: CallbackContext) -> int:
    ...
    if country_code and ip:
        ...
        if country_code in db.ipv4_data:
            name, flag, ips = db.ipv4_data[country_code]
            if ip in ips:
                ips.remove(ip)
                db.ipv4_data[country_code] = (name, flag, ips)
                db.save_database()
